...
export function ProjectsSection({
  ...
  return (
    <ul className="overflow-hidden rounded-xl border border-border bg-surface divide-y divide-border">
      {projects.map((p) => {
        const status = p.status as import('@/lib/queries/admin').ProjectStatus;
        return (
          <li key={p.id}>
            <Link
              href={`/admin/projects/${p.id}`}
              className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 transition-colors hover:bg-surface-2/50"
            >
                ...
                <p className="mt-0.5 font-mono text-[10px] uppercase tracking-[0.16em] text-ink-subtle">
                  <span
                    className={cn(
                      ...
                      PROJECT_STATUS_DOT[status] ?? 'bg-ink-subtle',
                    )}
                    aria-hidden
                  />
                  {PROJECT_STATUS_LABEL[status] ?? p.status.replace('_', ' ')}
                  {p.startDate || p.endDate ? (
                    <span className="ml-2 normal-case tabular-nums text-ink-subtle">
